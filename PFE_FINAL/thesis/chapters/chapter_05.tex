...
The prototype was developed and deployed on the following infrastructure:
...
The choice of CPU-only deployment was driven by the DNSI's current infrastructure constraints. The 4-bit quantization of Mistral 7B (\texttt{mistral-7b-instruct-v0.2.Q4\_K\_M.gguf}) reduces memory requirements from $\sim$14 GB (FP16) to $\sim$4.5 GB, with a perplexity degradation reported to be under 3\% relative to the full-precision model for this quantization level \citep{GerganovLlamaCpp2023}; 4-bit precision has moreover been shown to be near-universally optimal in the accuracy--model-bits trade-off \citep{Dettmers2023}.
...
The MCP Context Server is implemented using the official Anthropic MCP Python SDK. The server exposes the \texttt{search\_documents} tool and the \texttt{rag\_response} prompt template over the streamable-HTTP transport (initially HTTP/SSE; see the proxy challenge below).
...
async def search_documents(name: str, arguments: dict) -> list[TextContent]:
    ...
    scores = cross_encoder.predict(
        [(query, c["text"]) for c in authorized_chunks]
    )
    reranked = sorted(
        zip(authorized_chunks, scores), key=lambda x: x[1], reverse=True
    )[:5]
...
\paragraph{Engineering challenge: MCP transport configuration behind corporate proxy.} The HTTP/SSE transport initially used by the MCP server was broken by the corporate proxy, which buffers and inspects long-lived streaming responses and thereby delays or drops Server-Sent Events. The resolution was to configure the MCP server to use the \texttt{streamable-http} transport variant \citep{MCPSpec2025}, which tolerates proxy buffering, rather than the default SSE transport.
...
Corporate proxy blocking MCP SSE & Proxy buffering breaks SSE streaming & Switch to streamable-http transport & Resolved \\
...
Mistral 7B inference latency (CPU) & 25--45s per response on CPU (estimated) & 4-bit GGUF quantization; response streaming & Mitigated \\
\bottomrule
\end{tabularx}
\end{table}

The most significant unresolved challenge is LLM inference latency on CPU. The 4-bit quantized Mistral 7B generates at approximately 3--8 tokens/second on the available CPU infrastructure, which translates to estimated response times of 25--45 seconds for responses of 150--300 tokens. This exceeds NFR01 (P95 $\leq$ 5 seconds) and is the primary performance gap in the current deployment. GPU inference would reduce generation latency to 1--3 seconds. This limitation is discussed in the system performance evaluation (Chapter~\ref{chap:evaluation}) and in the limitations analysis (Chapter~\ref{chap:discussion}).
...
To support reproducibility of the planned experimental evaluation, the following artifacts are provided in Annex~C:
...
This chapter has documented the implementation of AntiGravity across its four architectural layers. The technology stack is grounded in Python, Streamlit, FAISS, sentence-transformers, and the MCP Python SDK. Seven significant engineering challenges were encountered during development; five were fully resolved, one (the absence of in-place deletion in FAISS flat indexes) is an accepted limitation, and one (CPU inference latency) was mitigated but remains the primary performance gap of the current infrastructure environment. The implementation is reproducible through the artifacts provided in the appendices. The following chapter presents the experimental evaluation of the implemented system.
