...
\paragraph{SC3 --- Application of DSR to enterprise AI evaluation.} The mémoire demonstrates that Design Science Research provides a principled framework for research that simultaneously produces a system artifact and empirically evaluates it. The three-cycle DSR model (Relevance, Design, Rigor) maps cleanly onto the research phases of this work and is recommended as the methodology of choice for applied enterprise AI research.
...
This mémoire establishes both the technical foundation and the scientific positioning for a CIFRE doctoral research program. Three open scientific questions, identified through the limitations of this mémoire, constitute a coherent doctoral research agenda:
...
The AntiGravity system addresses a real operational problem: the gap between the organizational knowledge embedded in enterprise documentation and the practical ability of employees to access and use that knowledge. The RAG+MCP architecture presented in this mémoire is one technically grounded response to that problem. Its implementation and deployment demonstrate feasibility under real DNSI operational constraints --- corporate proxy, OAuth 2.0 authentication, mixed-language corpus, CPU-only deployment --- and identify the primary path to production readiness: GPU infrastructure for inference latency and real-time delta synchronization for corpus freshness.

The contributions of this mémoire are bounded by the constraints of a Master's PFE: a single organization, a proof-of-concept deployment, and a time-limited evaluation. These boundaries are acknowledged explicitly in Chapter~\ref{chap:discussion}. Within these boundaries, the mémoire addresses each research question with a documented, replicable protocol and artifacts.
