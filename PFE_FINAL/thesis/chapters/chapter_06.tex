...
This chapter reports the evaluation of the AntiGravity system across the four dimensions defined in Chapter~\ref{chap:methodology}: retrieval quality, generation quality, system performance, and user acceptance. Each section presents the results for the relevant hypotheses and interprets their significance --- including where results fall short of targets.
...
Two independent annotators labelled the ground-truth relevant chunks for each question. Inter-annotator agreement was measured using Cohen's \$\kappa\$; pairs that fell below \$\kappa = 0.6\$ were adjudicated and revised before inclusion in the benchmark.
...
Retrieval is evaluated on Conditions B and C (Condition A has no retrieval component). Results are reported over the answerable questions in the benchmark. A BM25 lexical baseline provides a reference point for assessing the contribution of dense semantic search.
...
Table~\ref{tab:ragas-results} reports RAGAS generation quality across all three conditions on the answerable questions.
...
%  (A2) direct FAISS call without MCP wrapper vs. (A4) same FAISS call through MCP transport.
%  The difference isolates MCP protocol overhead from FAISS search time.
...
The System Usability Scale (SUS) is administered to all participants after a single-session evaluation using the five task scenarios of Appendix~\ref{app:user-study}. SUS scores are computed using the standard Brooke (1996) formula and interpreted against the adjective rating scale of \citet{Brooke1996SUS}.
...
TAM Perceived Usefulness (PU) and Perceived Ease of Use (PEOU) are measured using the instruments in Appendix~\ref{app:user-study}. SharePoint search satisfaction is collected as a baseline using the same 7-point Likert scale applied to the PU items.
...
\paragraph{Hypothesis H5 (user acceptance SUS $\geq$ 68 and TAM PU significantly higher than SharePoint baseline):} A SUS score above 68 corresponds to the "good" classification and represents the minimum acceptable adoption threshold. The TAM comparison tests whether users perceive AntiGravity as more useful than the current SharePoint search workflow.
