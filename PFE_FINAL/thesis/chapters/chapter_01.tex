...
The digitalization of organizational processes has generated a large and continuously growing volume of business documentation. Technical specifications, procedural guides, project reports, regulatory frameworks, meeting minutes, and operational manuals accumulate on enterprise collaborative platforms, encoding years of expertise, decision history, and operational knowledge. Yet despite the availability of this documentation, organizations consistently struggle to translate \textit{stored} information into \textit{accessible} information.

The symptoms of this gap are well documented in the enterprise knowledge management literature \citep{Alavi2001, Davenport1998}. Employees spend a disproportionate fraction of their working time searching for information rather than using it: the McKinsey Global Institute estimates that the average interaction worker spends nearly 20\% of the workweek looking for internal information or tracking down colleagues who can help with specific tasks \citep{McKinsey2012}. Complex queries — those requiring synthesis across multiple documents, historical context, or domain-specific reasoning — are particularly poorly served by conventional search interfaces. The result is a form of knowledge friction — the resistance that organizational structures, technical interfaces, and information architectures impose between the knowledge that exists and the knowledge that can be practically applied, echoing the frictions that \citet{Davenport1998} identify in organizational knowledge markets.

Microsoft SharePoint, an enterprise content management platform that surpassed 200 million monthly active users in the cloud as early as 2020 \citep{Spataro2020}, is a paradigmatic example of this tension. SharePoint provides robust document storage, versioning, and access control capabilities, but its search functionality is fundamentally lexical: it matches keywords without understanding intent, context, or semantic proximity. A user querying "what are the security requirements for deploying a new application?" may receive thousands of matching documents, ranked by keyword frequency, with no mechanism for synthesizing a coherent, sourced answer across the relevant subset.

This mémoire originates in a concrete instantiation of this problem. The \gls{dnsi} — the information systems directorate of a large organization — maintains an extensive documentation corpus on its SharePoint platform (SharePoint \gls{cerp}). This corpus includes technical architecture documents, security policies, operational procedures, and project deliverables. Business units within the DNSI face daily challenges in locating, interpreting, and synthesizing information from this corpus, a friction that imposes measurable operational and decision-making costs.
...
These three limitations converge to make direct LLM deployment unsuitable for enterprise knowledge management in most regulated environments.
...
\paragraph{Technical Contribution 3 (TC3):} A Docker Compose deployment package enabling one-command deployment of the full AntiGravity stack (MCP server, Streamlit client, ingestion worker) on a corporate network with proxy support.

\paragraph{Technical Contribution 4 (TC4):} A deployment and configuration guide documenting the Entra ID application registration, OAuth 2.0 delegated permission configuration, and proxy settings required to operate AntiGravity in a corporate environment.
...
This mémoire addresses the design, implementation, and evaluation of a conversational assistant for enterprise document access. The following boundaries apply:
...
%------------------------------------------------------------------------------
\section{Structure of the Mémoire}
\label{sec:intro-structure}
%------------------------------------------------------------------------------

The remainder of this mémoire is organized as follows:

\textbf{Chapter~\ref{chap:background}} establishes the theoretical and empirical foundations of the work through a comprehensive review of the literature on LLMs, RAG architectures, vector search, enterprise knowledge management, the Model Context Protocol, and enterprise AI security. It concludes by identifying the research gap this mémoire fills.
...
\textbf{Chapter~\ref{chap:conclusion}} synthesizes the contributions of the mémoire, provides explicit answers to each research question, and outlines the directions for future work, including a proposed research agenda for CIFRE doctoral continuation.
